...
class RPCserviceHandler : virtual public RPCserviceIf {
 public:
  ...
  void getWebPage(responseData& _return, const std::string& url) {
    ...
    size_t rand_size = WWW_MIN_CACHE_LEN;
    clock_t start, end;
    try {
  //    printf("Enter try.\n");
      char *pageContent = (char *) malloc(1024);
      start = clock();
      ...
      if (pageContent == NULL){
        ...
        gtcache_set(url, chunk.memory, rand_size); 
      }
      else{
        // A hit!
        printf("A hit for %s\n", url.c_str());
        _return.isSuccess = true;
        _return.webPage = std::string(pageContent, pageContent + rand_size);
      }
