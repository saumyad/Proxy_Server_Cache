...
int gtcache_set(std::string key, void* value, size_t val_size){
	...
	if (location != std::make_pair(x, y)){
		...
		while(used_mem + val_size > cache_capacity || available_ids.size() == 0){
			//Choose one entry from eviction_queue, at random.
			std::map< std::pair<long, int>, bool >::iterator pq = eviction_queue.begin();
			int qsize = eviction_queue.size();	//TODO: Check if this will work.
			int numToEvict = rand() % qsize + 1;
			int i = 1;
			for (std::map< std::pair<long, int>, bool >::iterator it=eviction_queue.begin(); it!=eviction_queue.end(); ++it) {
    			if(i == numToEvict) {
    				pq = it;
    				break;
    			}
